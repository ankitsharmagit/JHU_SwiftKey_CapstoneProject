...
This app uses english text files (Twitter feed, blogs and news), samples them and builds Corpus corpus by tokenization, removing Stopwords, punctuation.

Analyze a large corpus of text documents to discover the distribution and relationship between the words, tokens, and phrases in the text.

Builds predictive model using n-gram stupid backoff model (Fourgram is first used; If no Fourgram is found, back off to Trigram; If no Trigram is found, back off to Bigram; If no Bigram is found, back off to the most common word ‘the’).
...
The link for the Word Prediction App is: https://ankitsh.shinyapps.io/NextWordPredictor/

Just type in a few words, click SUBMIT botton, wait a few seconds, and a table showing five predicted words with their corresponding scores will pop up.

Click the link below to see the source code: 
